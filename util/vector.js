  ...
  let result = args.reduce(
    (result, val, index) => {
      if ("prev" in result && isNaN(val)) {
        throw new Error(
          `Mismatched vector point pairing: {${type[0]}: ${result.prev}, ${
            type[1]
          }: NaN(${JSON.stringify(val)})}`
        );
      }
...
export default class Vector {
  ...
  toString(listMagAng) {
    return `Vector{x: ${this.x}, y: ${this.y +
      (listMagAng ? `, magnitude: ${this.magnitude}, angle: ${this.angle}` : "")}}`;
  }
  add(...vec) {
  ...
  subtract(...vec) {
    return Vector.add(
      ...
      ...reducePoints(...vec)
        .asVectors()
        .map(Vector.invert)
    ); //Allow calling via Vector.subtract()
  }
  invert() {
